"""
PropManager — Commercial Real Estate Models
Inspired by SAP RE-FX architecture.

Hierarchy:
  Architectural View : BusinessEntity → Building → Floor → Space
  Usage View         : RentalObject (groups Spaces) → PooledSpace
  Contract Layer     : BusinessPartner → Contract → ContractObject → Condition
  Settlement Layer   : ParticipationGroup → SettlementUnit → CostCollector
  Sales Layer        : SalesRule → SalesDeclaration
  Security Deposits  : DepositContract (separate from main lease)
  Vacancy            : VacancyPosting
"""

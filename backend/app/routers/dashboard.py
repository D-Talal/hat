...
from app.models.retail import (
    BusinessEntity, Building, RentalObject, SpaceStatus,
    Contract, ContractStatus, Condition, Invoice, MaintenanceRequest
)
...
def get_finance(
    ...
    # ── Commercial (invoices from re_invoices) ──
    ...
    if module in ("hotel", "all"):
        def booking_sum(filters):
            q = db.query(func.coalesce(func.sum(Booking.total_amount), 0)).filter(*filters)
            return float(q.scalar())
    ...
    # Merge monthly data
    all_months = set(commercial_monthly.keys()) | set(hotel_monthly_map.keys())
    for ym in sorted(all_months):
        amount = commercial_monthly.get(ym, 0.0) + hotel_monthly_map.get(ym, 0.0)
...
def get_occupancy(
    ...
    if module in ("commercial", "all"):
        total_objects = db.query(func.count(RentalObject.id)).scalar() or 0
        occupied = db.query(func.count(RentalObject.id)).filter(
            RentalObject.status == SpaceStatus.occupied
        ).scalar() or 0
    ...
    # Occupancy trend last 6 months (commercial — based on released contracts)
    if module in ("commercial", "all"):
        trend = []
...
def get_assets(
    ...
    if module in ("commercial", "all"):
        # Use BusinessEntity as the top-level commercial asset
        result["total_properties"] = db.query(func.count(BusinessEntity.id)).scalar() or 0
        result["total_units"] = db.query(func.count(RentalObject.id)).scalar() or 0

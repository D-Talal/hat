...
from app.models.retail import BusinessEntity, Contract, Invoice
...
def map_stats(module: str = "hotel", db: Session = Depends(get_db), u=Depends(get_current_user)):
    ...
    if module == "hotel":
        ...
        # Commercial module — use BusinessEntity as the geographic unit
        entities = db.query(BusinessEntity).all()
        for e in entities:
            country = (e.country or "Unknown").strip()
            continent = (e.continent or CONTINENT_MAP.get(country, "Other")).strip()

            # Use annual_revenue if set, else sum paid invoices for contracts under this entity
            revenue = float(e.annual_revenue or 0)
            if not revenue:
                paid = (db.query(func.sum(Invoice.amount))
                        .join(Contract, Invoice.contract_id == Contract.id)
                        .filter(
                            Contract.business_entity_id == e.id,
                            Invoice.status == "paid"
                        )
                        .scalar()) or 0
                revenue = float(paid)

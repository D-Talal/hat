...
def update_hotel(id: int, data: HotelCreate, db: Session = Depends(get_db)):
...
def update_guest(id: int, data: GuestCreate, db: Session = Depends(get_db)):
...
def list_bookings(room_id: Optional[int] = None, guest_id: Optional[int] = None, db: Session = Depends(get_db)):
    q = db.query(Booking)
    if room_id: q = q.filter(Booking.room_id == room_id)
    if guest_id: q = q.filter(Booking.guest_id == guest_id)
    return q.all()

@router.post("/bookings", response_model=BookingOut)
def create_booking(data: BookingCreate, db: Session = Depends(get_db)):
    obj = Booking(**data.dict())
    db.add(obj); db.commit(); db.refresh(obj); return obj

@router.put("/bookings/{id}", response_model=BookingOut)
def update_booking(id: int, data: BookingCreate, db: Session = Depends(get_db)):
...
# --- Dashboard Stats ---
@router.get("/stats")
def hotel_stats(db: Session = Depends(get_db)):

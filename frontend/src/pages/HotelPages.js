...
const RoomForm = ({ form, setForm }) => {
  ...
  useEffect(() => { hotel.hotels.list().then(r => setHotels(r.data)).catch(() => {}); }, []);
  return <>
    <Select label="Hotel" value={form.hotel_id || ''} onChange={s('hotel_id')} required>
...
const GuestForm = ({ form, setForm }) => {
  ...
  return <>
    ...
    <Select label="Nationality" value={form.nationality || ''} onChange={s('nationality')}>
      <option value="">— Select Country —</option>
      {COUNTRIES.map(c => <option key={c.name} value={c.name}>{c.name}</option>)}
    </Select>
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
...
const BookingForm = ({ form, setForm }) => {
  ...
  useEffect(() => {
    hotel.rooms.list().then(r => setRooms(r.data)).catch(() => {});
    hotel.guests.list().then(r => setGuests(r.data)).catch(() => {});
  }, []);
  return <>
    <Select label="Room" value={form.room_id || ''} onChange={s('room_id')} required>
      <option value="">— Select Room —</option>
      {rooms.map(r => <option key={r.id} value={r.id}>Room {r.room_number} — {r.room_type} ({r.status})</option>)}
    </Select>
    <Select label="Guest" value={form.guest_id || ''} onChange={s('guest_id')} required>
      <option value="">— Select Guest —</option>
      {guests.map(g => <option key={g.id} value={g.id}>{g.first_name} {g.last_name} {g.email ? `(${g.email})` : ''}</option>)}
    </Select>
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>

...
const PropForm = ({ form, setForm }) => {
  ...
  return <>
    ...
    <Input label="Annual Revenue ($)" type="number" value={form.annual_revenue || ''} onChange={s('annual_revenue')} />
  </>;
};
export function RetailProperties() {
  return <CrudPage title="Properties" sub="Commercial property portfolio" api={retail.properties}
    emptyForm={{ name: '', address: '', city: '', country: '', continent: '', total_area_sqft: '', annual_revenue: '' }}
    columns={[
      { key: 'name', label: 'Property Name' },
      { key: 'city', label: 'City' },
      { key: 'country', label: 'Country' },
      { key: 'total_area_sqft', label: 'Area', render: v => v ? `${Number(v).toLocaleString()} sqft` : '—' },
      { key: 'annual_revenue', label: 'Annual Revenue', render: v => v ? `$${Number(v).toLocaleString()}` : '—' },
...
const UnitForm = ({ form, setForm }) => {
  ...
  useEffect(() => { retail.properties.list().then(r => setProperties(r.data)).catch(() => {}); }, []);
  return <>
    <Select label="Property" value={form.property_id || ''} onChange={s('property_id')} required>
    ...
    <Input label="Unit Type" value={form.unit_type || ''} onChange={s('unit_type')} />
...
export function RetailUnits() {
  return <CrudPage title="Units" sub="Rentable spaces and status" api={retail.units}
    emptyForm={{ property_id: '', unit_number: '', floor: '', area_sqft: '', unit_type: '', status: 'available', monthly_rent: '' }}
    columns={[
      { key: 'unit_number', label: 'Unit' }, { key: 'unit_type', label: 'Type' }, { key: 'floor', label: 'Floor' },
...
const TenantForm = ({ form, setForm }) => {
  ...
  useEffect(() => { retail.units.list().then(r => setUnits(r.data)).catch(() => {}); }, []);
  return <>
    <Select label="Unit" value={form.unit_id || ''} onChange={s('unit_id')} required>
...
export function RetailTenants() {
  return <CrudPage title="Tenants" sub="Tenant leases and contacts" api={retail.tenants}
    ...
    columns={[
      { key: 'business_name', label: 'Business' }, { key: 'contact_name', label: 'Contact' },
      { key: 'email', label: 'Email' }, { key: 'monthly_rent', label: 'Rent/mo', render: v => v ? `$${Number(v).toLocaleString()}` : '—' },
      { key: 'lease_end', label: 'Lease End' }, { key: 'lease_status', label: 'Status', badge: true },
...
const InvoiceForm = ({ form, setForm }) => {
  ...
  useEffect(() => { retail.tenants.list().then(r => setTenants(r.data)).catch(() => {}); }, []);
  return <>
    <Select label="Tenant" value={form.tenant_id || ''} onChange={s('tenant_id')} required>
      <option value="">— Select Tenant —</option>
      {tenants.map(t => <option key={t.id} value={t.id}>{t.business_name} ({t.contact_name || 'No contact'})</option>)}
    </Select>
...
export function RetailInvoices() {
  return <CrudPage title="Invoices" sub="Rent payments and balances" api={retail.invoices}
    ...
    columns={[
      { key: 'tenant_id', label: 'Tenant ID' }, { key: 'amount', label: 'Amount', render: v => v ? `$${Number(v).toLocaleString()}` : '—' },
      { key: 'due_date', label: 'Due Date' }, { key: 'paid_date', label: 'Paid', render: v => v || '—' },
      { key: 'status', label: 'Status', badge: true },
...
export function RetailMaintenance() {
  return <CrudPage title="Maintenance" sub="Track and resolve requests" api={retail.maintenance}
    ...
    columns={[
      ...
      { key: 'reported_by', label: 'Reported By' }, { key: 'status', label: 'Status', badge: true },

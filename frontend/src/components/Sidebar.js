...
export default function Sidebar() {
  ...
  const navItems = [
    ...
    { section: t.nav.commercial, items: [
      { path: '/retail/properties', label: t.nav.properties, icon: '▦' },
      { path: '/retail/units', label: t.nav.units, icon: '⊞' },
      { path: '/retail/tenants', label: t.nav.tenants, icon: '◉' },
      { path: '/retail/invoices', label: t.nav.invoices, icon: '◇' },
      { path: '/retail/maintenance', label: t.nav.maintenance, icon: '⚙' },
    ]},
    ...
    <nav style={{
      ...
      {user && (
        <div style={{ padding: '16px 24px', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ overflow: 'hidden' }}>
            ...
            <div style={{ fontSize: 11, color: roleColors[user.role] || 'var(--gold)', textTransform: 'uppercase', fontWeight: 700, letterSpacing: '0.06em' }}>
              {user.role}
            </div>
          </div>
        </div>
      ...
      <div style={{ padding: '12px 24px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', fontWeight: 500, letterSpacing: '0.05em' }}>
          ...
          >
            <span style={{
              ...
              color: language === 'en' ? '#1a1a2e' : 'rgba(255,255,255,0.4)',
              transition: 'all 0.2s',
            }}>EN</span>
            <span style={{
              fontSize: 12, fontWeight: 700, padding: '3px 10px', borderRadius: 20,
              background: language === 'fr' ? 'var(--gold)' : 'transparent',
              color: language === 'fr' ? '#1a1a2e' : 'rgba(255,255,255,0.4)',
              transition: 'all 0.2s',
            }}>FR</span>
          </button>
        </div>

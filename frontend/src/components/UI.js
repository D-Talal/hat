...
const s = {
  card: { background: 'var(--surface)', borderRadius: 'var(--radius)', boxShadow: 'var(--shadow)', padding: '24px', border: '1px solid var(--border)' },
  btn: { display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '10px 20px', borderRadius: '8px', fontFamily: 'DM Sans', fontSize: '14px', fontWeight: 500, cursor: 'pointer', border: 'none', transition: 'all 0.2s' },
  input: { width: '100%', padding: '10px 14px', borderRadius: '8px', border: '1.5px solid var(--border)', fontFamily: 'DM Sans', fontSize: '14px', background: 'var(--cream)', color: 'var(--ink)', outline: 'none', transition: 'border-color 0.2s' },
...
export const StatCard = ({ label, value, sub, color, icon }) => (
  <div style={{ background: color || 'var(--ink)', borderRadius: 'var(--radius)', padding: '24px', color: 'white', position: 'relative', overflow: 'hidden' }}>
...
export const Badge = ({ status }) => {
  const colors = {
    ...
    admin: { bg: '#FEF3C7', color: '#92400E' }, manager: { bg: '#DBEAFE', color: '#1E40AF' },
    viewer: { bg: '#F3F4F6', color: '#374151' }, accountant: { bg: '#D1FAE5', color: '#065F46' },
  };
  const c = colors[status] || { bg: '#F3F4F6', color: '#374151' };
  return <span style={{ ...s.badge, background: c.bg, color: c.color }}>{status?.replace(/_/g, ' ')}</span>;
};

export const Table = ({ cols, rows, onEdit, onDelete, canEdit, canDelete }) => (
  <div style={{ overflowX: 'auto' }}>
    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
      <thead>
        <tr>
          {cols.map(c => <th key={c.key} style={s.th}>{c.label}</th>)}
          {(canEdit || canDelete) && <th style={s.th}>Actions</th>}
        </tr>
      </thead>
      <tbody>
        {rows.length === 0 ? (
            ...
            {(canEdit || canDelete) && (
              <td style={s.td}>
                <div style={{ display: 'flex', gap: 8 }}>
                  {canEdit && <Btn variant="ghost" onClick={() => onEdit(row)} style={{ padding: '6px 12px', fontSize: 12 }}>Edit</Btn>}
                  {canDelete && <Btn variant="danger" onClick={() => onDelete(row.id)} style={{ padding: '6px 12px', fontSize: 12 }}>Del</Btn>}
                </div>
              </td>
            )}
          </tr>
...
export const Modal = ({ title, children, onClose }) => (
  <div style={{
    ...
    {/* Modal box — centered with scroll */}
    <div style={{
      position: 'relative',
      background: 'white',
      borderRadius: 16,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, position: 'sticky', top: 0, background: 'white', paddingBottom: 16, borderBottom: '1px solid var(--border)', zIndex: 1 }}>
        <h3 style={{ fontSize: 22 }}>{title}</h3>
        <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: 24, cursor: 'pointer', color: 'var(--slate)', lineHeight: 1, padding: '0 4px' }}>×</button>
      </div>
